...
Ext.define('Ext.ux.grid.feature.MultiGrouping', {
    ...
    // Swami : Oct 16 
    // changed recordIndex to rowIndex
    // summmaryRecord to values.summaryRecord
    groupTpl: [
        '{%',
            'var me = this.groupingFeature;',
            'me.setupRowData(values.record, values.rowIndex, values,values.depth);',
                ...
                '<td class="' + Ext.baseCSSPrefix + 'group-hd-container" colspan="{columns.length}" >',
                    '<tpl if="isFirstRow">',
                        ...
                        '<tpl for="groupInfos">' +
		                    ...
		                    '<tpl if="values.summaryRecord && !lastGroup">',
    ...
    getGroupName: function(element) {
        ...
        if (targetEl) {
        	...
        	if(targetEl.groupPath){
        		return targetEl.groupPath ;
        	}else {
        		parts = targetEl.id.split(view.id + '-hd-');
                if (parts.length === 2) {
                    return Ext.htmlDecode(parts[1]);
    ...
    setupRowData: function(record, idx, rowValues,depth) {
        var me = this,
            ...
            groupName, prev, next,lastGrouper,
            grouperLength,groupInfos = new Array(),
            ...
            if (rowValues.isFirstRow) {
            	...
            	for(var i=k;i<grouperLength;i++){
            		...
            		if(i==0 || me.isExpanded(lastPath)){
	        			...
	        			header = me.getGroupedHeader(groupInfo.groupField);
	        			groupInfo.columnName = header ? header.text : tempGrouper.property;
	        			groupInfo.depth = i ;
	        			// Swami : Oct 16 (groupId from groupPath instead of groupInfo.name)
	        			groupInfo.groupId = me.createGroupId(groupPath);
	        			groupInfo.groupPath = groupPath;
	        			groupInfo.collapsibleCls = me.collapsible ? me.collapsibleCls : me.hdNotCollapsibleCls;
	        			groupInfo.lastGroup = i== grouperLength-1 ? true : false ;
                    ...
                    // Swami : Oct 16 (children row uncommented)
	                    if (store.buffered) {
	                    	groupInfo.rows = groupInfo.children = [];
	                    } else {
	                    	groupInfo.rows = groupInfo.children = me.getRecordGroup(record, i).children;
	                    }
